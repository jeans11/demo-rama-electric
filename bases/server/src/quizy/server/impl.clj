...
(defn template
  "Takes a `string` and a map of key-values `kvs`.
   Replace all instances of `$key$` by value in `string`"
  [string kvs]
...
(def authenticate (constantly true))
...
(defn- http-middleware [resources-path manifest-path]
  ...
  (-> not-found-handler
      ;; 4. otherwise fallback to default page file
      (wrap-index-page resources-path manifest-path)
      ;; 3. serve static file from classpath
      (wrap-resource resources-path)
      ;; 2. detect content (e.g. for index.html)
      (wrap-content-type)
      ;; 1. intercept electric websocket
...
(defn start-server!
  ...
  (try
    (let [server (ring/run-jetty (http-middleware resources-path manifest-path)
      ...
      (println "App server available at" (str "http://" (:host config) ":" final-port "\n"))
      server)
    (catch IOException err
      ...
      (if (instance? BindException (ex-cause err))
        (do
          (log/info "STARTING")
          (println "Port" port "was not available, retrying with" (inc port))
